...
const static std::vector<std::vector<std::string>> BN_FOLDING_PATTERNS = {
    ...
    {"DepthwiseConv2dNative", "Broadcast", "Add", "BatchNormInference"},
    {"DepthwiseConv2dNative", "Add", "BatchNormInference"},
    {"DepthwiseConv2dNative", "BatchNormInference"}};
...
private:
                    ...
                    const std::string& identifier)
    {
                     ...
                     {"DepthwiseConv2dNative", "Broadcast", "Add", "BatchNormInference"}))
        {
            auto conv_node = matched[0]->node;
            auto conv_node_op_ptr =
                std::dynamic_pointer_cast<nnfusion::op::GenericOp>(conv_node->get_op_ptr());
            auto broadcast_node = matched[1]->node;
            auto add_node = matched[2]->node;
            auto bn_node = matched[3]->node;
            auto bn_node_op_ptr =
            ...
            if (conv_weight_const_ptr == nullptr)
            {
                NNFUSION_LOG(NNFUSION_WARNING) << "DepthwiseConv2dNative weight is not constant. "
                                                  "Runtime_const_folding pass may solve "
                                                  "this problem. Fallback.";
                return -1;
            }
            ...
            if (conv_bias_const_ptr == nullptr)
            {
                NNFUSION_LOG(NNFUSION_WARNING) << "DepthwiseConv2dNative bias is not constant. "
                                                  "Runtime_const_folding pass may solve "
                                                  "this problem. Fallback.";
                return -1;
            }
            ...
            return add_node->get_id();
                 ...
                 std::vector<std::string>({"DepthwiseConv2dNative", "Add", "BatchNormInference"}))
        {
            auto conv_node = matched[0]->node;
            auto conv_node_op_ptr =
                std::dynamic_pointer_cast<op::GenericOp>(conv_node->get_op_ptr());
            auto add_node = matched[1]->node;
            auto bn_node = matched[2]->node;
            auto bn_node_op_ptr =
                std::dynamic_pointer_cast<op::BatchNormInference>(bn_node->get_op_ptr());
            ...
            std::vector<double> conv_bias_tmp = ExtractConstantData(conv_bias_const_ptr, dtype);
            auto conv_output_shape = conv_node->get_output_shape(0);
            bool is_nhwc = (conv_node_op_ptr->localOpConfig.getRoot()["data_format"] == "NHWC");
            if (is_nhwc)
            {
                for (auto c = 0; c < bn_bias.size(); c++)
                {
            ...
            return new_add_gnode->get_id();
                 ...
                 std::vector<std::string>({"DepthwiseConv2dNative", "BatchNormInference"}))
        {
            auto conv_node = matched[0]->node;
            auto conv_node_op_ptr =
                std::dynamic_pointer_cast<op::GenericOp>(conv_node->get_op_ptr());
